...
void blanco(){
  for(int i=0;i<8;i++){
   ...
   lecturas[i] = sensorValues[i] * amplificacion;
...
int lectura(void){
  ...
  {
  lecturas[i] = sensorValues[i] * amplificacion;
  if(linea==0){if(lecturas[i]<=umbral[i]){digital[i]=0;}else{digital[i]=1;}}
  if(linea==1){if(lecturas[i]<=umbral[i]){digital[i]=1;}else{digital[i]=0;}}
